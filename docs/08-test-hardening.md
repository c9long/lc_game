...
| problem | the gap | after |
|---|---|---|
| `set-matrix-zeroes` | values span all of int32, so no marker value is safe — but a `-1` marker only shows when a real `-1` survives in a row and column that are *not* zeroed. Half of all cells were zero, so every row and column was wiped and the marker bug scored **42/42** | zeros are sparse and `-1` common among the rest. The first draft of this generator also dropped a bracket, passing the rows as separate arguments; the oracle crashed and the generator deleted the suite rather than write a bad one, which is the behaviour you want |
| `happy-number` | 1 and 7 are the only single-digit happy numbers. 1 catches a loop that records `n` as seen before testing for 1; 7 — directly, or from numbers like 1112 whose digit squares sum to it — catches one that stops at the first single digit. Drawn from 1..500, both bugs scored **42/42** | 1, 7, 1112 and 2111 drawn often |

Everything else already failed: a transpose that swaps each pair twice or rebinds the local name
in `rotate-image`, the missing last-row and last-column guards in `spiral-matrix`, the lost carry
...
### Batch 17: Math & Geometry (8/8 audited)
- [x] `rotate-image` — Medium
- [x] `spiral-matrix` — Medium
- [x] `set-matrix-zeroes` — Medium
- [x] `happy-number` — Easy
- [x] `plus-one` — Easy
- [x] `powx-n` — Medium
